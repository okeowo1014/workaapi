...
def planchoice(name):
    return Plans.objects.get(name=name)
...
def display_picture(request):
    ...
    if request.method == 'POST' and request.FILES:
        ...
        employee.display_picture = 'http://127.0.0.1:8000/{}'.format(
...
def upload_cv(request):
    ...
    if request.method == 'POST' and request.FILES:
        ...
        if get_mime_type(cv) == 'application/pdf':
            ...
            try:
                employee.save()
                adduserlog(request.user, 'uploaded cv')
...
def company_logo(request):
    ...
    if request.method == 'POST' and request.FILES:
        ...
        employer.company_logo = 'http://127.0.0.1:8000/{}'.format(
            '{}/{}'.format(fss.base_location, picture.name))
        try:
            employer.save()
...
@api_view(['POST'])
def planUpgrade(request, uid, plan, trans_id):
    plan = planchoice(plan)
    employer = get_employee_by_id(uid)
    employer.plan = plan
    employer.plan_active = True
    employer.save()
    PlanUpgradeRecord.objects.create(plan=plan, transaction=trans_id)
    serializer = PlansSerializer(employer.plan, many=True)
